...
def collect_statistic_data(club):
    with open('toptracer-scrapping/statistics_toptracer.json', 'r') as f:
        data = json.load(f)
    data_curated = []
    for shot in data:
...
def button_started(video, club, distance):
    ...
    swing = get_swing_video_inputs("front_swing.mp4", statistics,distance,club)
    positive =get_swing_video_positive_inputs("front_swing.mp4", statistics,distance,club)
