...
def generate_photo(user_name,user_tag,text, hashtags, link,profile_pic,image_tweet): 
    ...
    FONT_USER_INFO = ImageFont.truetype("font.woff2", 90, encoding="utf-8")
    FONT_TEXT = ImageFont.truetype("font.woff2", 110, encoding="utf-8")
    ...
    image_banner = Image.open("bottom_banner.png", 'r')
...
def gather_twitter_current_local_trends():
    ...
    # # WOEID for the location you are interested in (e.g., 1 is global; 23424977 is for the United States)
    # location_woeid = 1

    # # Get trends for the location
    # trends_result = api.get_place_trends(location_woeid)

    # # Extract trend names and store them in a list
    # trend_list = [trend['name'] for trend in trends_result[0]['trends']]
